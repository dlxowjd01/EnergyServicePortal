...
//	static Timestamp endListDt = null;
...
//		Timestamp endListDt = null;
		...
		if( ("15min".equals(period) && flag == 1) || ("hour".equals(period) && flag == 2) ) { // 실제 15분간격 or 예측 1시간간격
			dataMap = periodSetting1(resultList, selTermFrom, selTermTo, term, period, timestampStr, calculValStr, flag);
			
		} else if( ("15min".equals(period) && flag == 2) || ("30min".equals(period) && flag == 2) ) { // 예측 15분간격 or 예측 30분간격
			dataMap = periodSetting2(resultList, selTermFrom, selTermTo, term, period, timestampStr, calculValStr, flag, calculCnt);
			
		} else { // 실제 30분간격 이상 or 예측 1일간격 이상
			dataMap = periodSetting3(resultList, selTermFrom, selTermTo, term, period, timestampStr, calculValStr, flag, calculCnt);
		}
	 ...
	 * @return
	 */
	private static Map<String, Object> periodSetting1(List dataList, Timestamp selTermFrom, Timestamp selTermTo, String term,
			String period, String timestampStr, String calculValStr, int flag) {
		List newList = new ArrayList();
	 ...
	 * @return
	 */
	private static Map<String, Object> periodSetting2(List dataList, Timestamp selTermFrom, Timestamp selTermTo, String term,
...
//			Timestamp strTm = (Timestamp) map.get(timestampStr); // 증가시킬 timestamp
			Timestamp strTm = incStartDt; // 증가시킬 timestamp
			// 계산해야하는 횟수만큼 for문 실행
...
//				}
	 ...
	 * @return
	 */
	private static Map<String, Object> periodSetting3(List dataList, Timestamp selTermFrom, Timestamp selTermTo, String term,
			String period, String timestampStr, String calculValStr, int flag, int calculCnt) {
		List newList = new ArrayList();
