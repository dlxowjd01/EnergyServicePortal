<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
<%@ include file="/decorators/include/taglibs.jsp"%>
				<div class="row">
					<div class="col-lg-12">
						<h1 class="page-header fl">설비 구성</h1>
						<div class="time fr">
							<span>CURRENT TIME</span>
							<em class="currTime">${nowTime}</em>
							<span>DATA BASE TIME</span>
							<em class="dbTime">2018-07-27 17:01:02</em>
						  </div>
					</div>
					<div class="header_drop_area col-lg-2 w_type">
						<div class="dropdown">
							<button class="btn btn-primary dropdown-toggle" type="button" data-toggle="dropdown">사업소#1
							<span class="caret"></span></button>
							<ul class="dropdown-menu dropdown-menu-form">
								<li class="on"><a href="#">사업소#1</a></li>
								<li><a href="#">사업소#2</a></li>
								<li><a href="#">사업소#3</a></li>
								<li><a href="#">사업소#4</a></li>
							  </ul>
						</div>
					</div>
				</div>
				<div class="row scroll">
					<div class="col-lg-12">
						<div class="row">
							<div class="col-lg-8">
								<div class="indiv clear">
									<div class="chart_top clear">
										<h2 class="ntit fl">인버터</h2>
										<div class="eq_icon fr">
											<span class="eq_normail">정상(39)</span> 
											<span class="eq_error">이상(1)</span>
											<span class="eq_alert">경고(2)</span>
										</div>
									</div>
									<ul class="eq_list scroll eq_li_type01">
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#1</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#2</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#3</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#4</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#5</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li class="st_error">
											<a href="#">삭제</a>
											<span class="tx">인버터#6</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#7</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#8</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#9</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#10</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#11</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#12</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#13</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#14</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#15</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#16</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li class="st_alert">
											<a href="#">삭제</a>
											<span class="tx">인버터#17</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li class="st_alert">
											<a href="#">삭제</a>
											<span class="tx">인버터#18</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#19</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#19</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">인버터#19</span>
											<span>1000kw</span>
											<em>560V 46.5A</em>
										</li>
										<li class="eq_add">
											<a href="#">추가</a>
										</li>
									</ul>
								</div>
							</div>
							<div class="col-lg-4">
								<div class="indiv eq_card">
									<div class="chart_top clear">
										<h2 class="ntit fl">인버터#1</h2>
									</div>
									<ul class="eq_card_ul clear">
										<li>
											<p class="t_ti">인버터 출력</p>
											<p class="t_value">409.4kWh</p>
										</li>
										<li>
											<p class="t_ti">태양광 입력</p>
											<p class="t_value">409.4kWh</p>
										</li>
										<li>
											<p class="t_ti">금일 발전량 출력</p>
											<p class="t_value">409.4kWh</p>
										</li>
									</ul>
									<div class="inv_sec_bx">
										<p class="inv_tit">인버터 현황</p>
										<ul class="isb_in clear">
											<li>
												<ul class="di_list">
													<li>
														<span class="di_li_tit">R전압</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">S전압</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">T전압</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">주파수</span>
														<span class="di_li_tx">409.5V</span>
													</li>
												</ul>
											</li>
											<li>
												<ul class="di_list">
													<li>
														<span class="di_li_tit">R전류</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">S전류</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">T전류</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">전력</span>
														<span class="di_li_tx">409.5V</span>
													</li>
												</ul>
											</li>
										</ul>
									</div>
									<div class="eq_btn_bx">
										<button type="submit" class="btn_type04">변경 이력 조회</button>
										<button type="submit" class="btn_type04">운영 이력 조회</button>
									</div>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-lg-8">
								<div class="indiv clear">
									<div class="chart_top clear">
										<h2 class="ntit fl">접속반</h2>
										<div class="eq_icon fr">
											<span class="eq_normail">정상(39)</span> 
											<span class="eq_error">이상(1)</span>
											<span class="eq_alert">경고(2)</span>
										</div>
									</div>
									<ul class="eq_list scroll clear eq_li_type02">
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#1</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#2</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#3</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#4</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#5</span>
											<em>560V 46.5A</em>
										</li>
										<li class="st_error">
											<a href="#">삭제</a>
											<span class="tx">접속반#6</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#7</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#8</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#9</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#10</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#11</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#12</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#13</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#14</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#15</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#16</span>
											<em>560V 46.5A</em>
										</li>
										<li class="st_alert">
											<a href="#">삭제</a>
											<span class="tx">접속반#17</span>
											<em>560V 46.5A</em>
										</li>
										<li class="st_alert">
											<a href="#">삭제</a>
											<span class="tx">접속반#18</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">접속반#19</span>
											<em>560V 46.5A</em>
										</li>
										<li class="eq_add">
											<a href="#">추가</a>
										</li>
									</ul>
								</div>
							</div>
							<div class="col-lg-4">
								<div class="indiv eq_card t2">
									<div class="chart_top clear">
										<h2 class="ntit fl">접속반#1</h2>
									</div>
									<ul class="eq_card_ul clear">
										<li>
											<p class="t_ti">DCU 전체 전압</p>
											<p class="t_value">409.5V</p>
										</li>
										<li>
											<p class="t_ti">DCU 전제 전류</p>
											<p class="t_value">409.5A</p>
										</li>
										<li>
											<p class="t_ti">전속반 식별자</p>
											<p class="t_value">-</p>
										</li>
									</ul>
									<div class="inv_sec_bx">
										<p class="inv_tit">접속반 현황</p>
										<ul class="isb_in clear">
											<li>
												<ul class="di_list">
													<li>
														<span class="di_li_tit">주변 온도</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">경사면 일사량</span>
														<span class="di_li_tx">10 kWh/㎡.day</span>
													</li>
													<li>
														<span class="di_li_tit">수평 일사량</span>
														<span class="di_li_tx">10 kWh/㎡.day</span>
													</li>
												</ul>
											</li>
										</ul>
									</div>
									<div class="eq_btn_bx">
										<button type="submit" class="btn_type04">변경 이력 조회</button>
										<button type="submit" class="btn_type04">운영 이력 조회</button>
									</div>
								</div>
							</div>
						</div>
						<div class="row">
							<div class="col-lg-8">
								<div class="indiv clear">
									<div class="chart_top clear">
										<h2 class="ntit fl">차단기</h2>
										<div class="eq_icon fr">
											<span class="eq_normail">정상(39)</span> 
											<span class="eq_error">이상(1)</span>
											<span class="eq_alert">경고(2)</span>
										</div>
									</div>
									<ul class="eq_list scroll clear eq_li_type03">
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#1</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#2</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#3</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#4</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#5</span>
											<em>560V 46.5A</em>
										</li>
										<li class="st_error">
											<a href="#">삭제</a>
											<span class="tx">차단기#6</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#7</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#8</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#9</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#10</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#11</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#12</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#13</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#14</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#15</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#16</span>
											<em>560V 46.5A</em>
										</li>
										<li class="st_alert">
											<a href="#">삭제</a>
											<span class="tx">차단기#17</span>
											<em>560V 46.5A</em>
										</li>
										<li class="st_alert">
											<a href="#">삭제</a>
											<span class="tx">차단기#18</span>
											<em>560V 46.5A</em>
										</li>
										<li>
											<a href="#">삭제</a>
											<span class="tx">차단기#19</span>
											<em>560V 46.5A</em>
										</li>
										<li class="eq_add">
											<a href="#">추가</a>
										</li>
									</ul>
								</div>
							</div>
							<div class="col-lg-4">
								<div class="indiv eq_card t3">
									<div class="chart_top clear">
										<h2 class="ntit fl">차단기#1</h2>
									</div>
									<ul class="eq_card_ul clear">
										<li>
											<p class="t_ti">현재 출력</p>
											<p class="t_value">409.4kWh</p>
										</li>
										<li>
											<p class="t_ti">주파수</p>
											<p class="t_value">62Hz</p>
										</li>
										<li>
											<p class="t_ti">금일 발전량 출력</p>
											<p class="t_value">4,856.3kWh</p>
										</li>
									</ul>
									<div class="inv_sec_bx">
										<p class="inv_tit">인버터 현황</p>
										<ul class="isb_in clear">
											<li>
												<ul class="di_list">
													<li>
														<span class="di_li_tit">R전압</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">S전압</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">T전압</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">주파수</span>
														<span class="di_li_tx">409.5V</span>
													</li>
												</ul>
											</li>
											<li>
												<ul class="di_list">
													<li>
														<span class="di_li_tit">R전류</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">S전류</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">T전류</span>
														<span class="di_li_tx">409.5V</span>
													</li>
													<li>
														<span class="di_li_tit">전력</span>
														<span class="di_li_tx">409.5V</span>
													</li>
												</ul>
											</li>
										</ul>
									</div>
									<div class="eq_btn_bx">
										<button type="submit" class="btn_type04">변경 이력 조회</button>
										<button type="submit" class="btn_type04">운영 이력 조회</button>
									</div>
								</div>
							</div>
						</div>
					</div>
				</div>
