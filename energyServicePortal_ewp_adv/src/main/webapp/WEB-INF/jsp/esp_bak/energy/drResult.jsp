...
// 				var queryEnd = new Date(endTime.setMinutes(endTime.getMinutes() + (new Date()).getTimezoneOffset()));
			...
			function getDRResultList(formData) {
				$.ajax({
					url: "/energy/getDRResultList.json",
					type: 'post',
					async: false, // 동기로 처리해줌
					data: formData,
					success: function (result) {
						...
						if (drList != null && drList.length > 0) {
							for (var i = 0; i < drList.length; i++) {
								...
								tbodyStr += '<tr>';
								tbodyStr += '<td>' + new Date(drStartDate).format("yyyy-MM-dd") + '</td>';
								tbodyStr += '<td>' + new Date(drStartDate).format("HH:mm") + " ~ " + new Date(drEndDate).format("HH:mm") + '</td>';
			...
			function refreshChartData(result) {
				...
				myChart.tooltip.options.formatter = function () {
					return '<b>' + this.series.name + '</b><br/>' + Highcharts.dateFormat('%Y-%m-%d %H:%M:%S ', new Date(this.x))
						+ '<br/><span style="color:#438fd7">' + numberComma(this.y) + ' kW</span>';
				};
				...
				<div class="row">
					...
					<div class="col-lg-10">
						<div class="indiv dr_chart">
							<c:set var="schGbn" value="energy_drResult" />
					...
					<div class="col-lg-12">
						<div class="indiv">
							<div class="tbl_top clear">
								...
								<ul class="fr">
									<li><a href="#;" class="save_btn" onclick="exportExcel('drResult');">데이터저장</a></li>
								</ul>
							</div>
							<div class="tbl_wrap">
