						...
						if ( !!window.Worker ) {
							...
							if (chgSheetList != null && chgSheetList.length > 0) {
								...
								for (var i = 0; i < chgSheetList.length; i++) {
									...
									dt_str += "<td>" + numberComma( (( isEmpty(reChgVal) ) ? "" : reChgVal) ) + "</td>"; // 충전량
									dt_str2 += "<td>" + numberComma( (( isEmpty(reDischgVal) ) ? "" : reDischgVal) ) + "</td>"; // 방전량

						            var tempDate; //timestamp 에 해당하는 날짜
						            var lastCol = dt_col; // 마지막 컬럼
						            if(SelTerm == "year" && periodd == "day") {
						            	tempDate = new Date(chgSheetList[i].std_timestamp);
									...
									if (dt_col_cnt === lastCol) {
										...
										dt_str += "<td>" + numberComma(toFixedNum(dt_str_totalVal, 2)) + "</td>"; // 충전량
					...
					<div class="col-lg-12">
						<div class="indiv">
							<div class="tbl_top clear">
								...
								<ul class="fr">
									<!-- <li><a href="#;" class="save_btn" onclick="excelDownload('ESS충방전량', event);">데이터저장</a></li> -->
									<li><a href="#;" class="save_btn" onclick="exportExcel('ESS충방전량');">데이터저장</a></li>
