...
<%@ include file="/decorators/include/taglibs.jsp"%>
		<script type="text/javascript">
			...
			function getDRRevenueTexList(formData) {
			    $.ajax({
			        url: "/billRevenue/getDRRevenueTexList.json",
			        type: 'post',
			...
			function getDRRevenueList(formData) {
			    $.ajax({
			        url: "/billRevenue/getDRRevenueList.json",
			        type: 'post',
			        async: false, // 동기로 처리해줌
			        data: formData,
			        success: function (result) {
						...
						if (sheetList != null && sheetList.length > 0) {
							for (var i = 0; i < sheetList.length; i++) {
								...
								dt_str2 += "<td>" + numberComma( (( isEmpty(reReductCap) ) ? "" : reReductCap) ) + "</td>"; // 감축이행용량 (kWh)
								dt_str3 += "<td>" + numberComma( (( isEmpty(reReductAmt) ) ? "" : reReductAmt) ) + "</td>"; // 감축인정용량 (kWh)
								dt_str4 += "<td>" + (( isEmpty(reReductCapPer) ) ? "" : reReductCapPer) + "</td>"; // 감축이행율 (%)
								dt_str5 += "<td>" + numberComma( (( isEmpty(reCapAmt)) ? "" : reCapAmt) ) + "</td>"; // 용량정산금 (KRW)
								dt_str6 += "<td>" + numberComma( (( isEmpty(reReductRewardAmt) ) ? "" : reReductRewardAmt) ) + "</td>"; // 감축정산금 (KRW)
								dt_str7 += "<td>" + numberComma( (( isEmpty(reTotalRewardAmt) ) ? "" : reTotalRewardAmt) ) + "</td>"; // 총 정산금액
								dt_str8 += "<td>" + numberComma( (( isEmpty(reCsmRewardAmt) ) ? "" : reCsmRewardAmt) ) + "</td>"; // 고객 할인금액
								dt_str9 += "<td>" + numberComma( (( isEmpty(reEwpRewardAmt) ) ? "" : reEwpRewardAmt) ) + "</td>"; // EWP 정산금액 (KRW)
								...
								if (dt_col_cnt === 12) {
									dt_str += "<td>" + numberComma(dt_str_totalVal) + "</td>";
									dt_str2 += "<td>" + numberComma(dt_str2_totalVal) + "</td>";
									dt_str3 += "<td>" + numberComma(dt_str3_totalVal) + "</td>";
									dt_str4 += "<td>" + ((reReductCapPer_cnt === 0) ? 0 : Math.round(dt_str4_totalVal / reReductCapPer_cnt)) + "</td>";
									dt_str5 += "<td>" + numberComma(dt_str5_totalVal) + "</td>";
									dt_str6 += "<td>" + numberComma(dt_str6_totalVal) + "</td>";
									dt_str7 += "<td>" + numberComma(dt_str7_totalVal) + "</td>";
									dt_str8 += "<td>" + numberComma(dt_str8_totalVal) + "</td>";
									dt_str9 += "<td>" + numberComma(dt_str9_totalVal) + "</td>";
						...
						if (chartList != null && chartList.length > 0) {
							...
							unit_format_bill(String(totDataSet), "drRevenueTot1", "KRW", "dr");
							unit_format_bill(String(totDataSet2), "drRevenueTot2", "KRW", "dr");
						}
			        }
			    });
			}
			...
			function drawData_table() {
				...
				if (drRevenue_data_pc.length > 0) {
					...
					for (var i = 0; i < dt_row; i++) {
						...
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit wht"><span>용량정산금 (KRW)</span></div></th>' + drRevenue_data_pc5[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit wht"><span>감축정산금 (KRW)</span></div></th>' + drRevenue_data_pc6[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit ct1"><span>총 정산금액 (KRW)</span></div></th>' + drRevenue_data_pc7[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit"><span>고객 할인금액 (KRW)</span></div></th>' + drRevenue_data_pc8[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit wht"><span>EWP 정산금액 (KRW)</span></div></th>' + drRevenue_data_pc9[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						...
						<div class="indiv">
							...
							<ul class="chart_total">
								<li class="ct1">
									...
									<div class="cval" id="drRevenueTot1"><span>0</span>KRW</div>
								</li>
								<li>
									<div class="ctit"><span>고객 할인금액</span></div>
									<div class="cval" id="drRevenueTot2"><span>0</span>KRW</div>
								</li>
							</ul>
						</div>
					</div>
										...
										tooltip: {
											formatter: function () {
												return '<b>' + this.series.name + '</b><br/>' + Highcharts.dateFormat('%Y-%m ', new Date(this.x)) +
													'월<br/><span style="color:#438fd7">' + this.y + ' KRW</span>';
											}
										},
