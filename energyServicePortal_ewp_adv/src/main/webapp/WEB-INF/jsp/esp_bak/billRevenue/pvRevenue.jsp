...
<%@ include file="/decorators/include/taglibs.jsp"%>
		<script type="text/javascript">
			...
			var pvRevenue_head_pc = []; //  표 데이터(헤더)
			var pvRevenue_data_pc = []; //  표 데이터
			var pvRevenue_data_pc2 = []; //  표 데이터
			var pvRevenue_data_pc3 = []; //  표 데이터
			var pvRevenue_data_pc4 = []; //  표 데이터
			var pvRevenue_data_pc5 = []; //  표 데이터
			var pvRevenue_data_pc6 = []; //  표 데이터
			...
			function getPVRevenueTexList(formData) {
			    $.ajax({
			        url: "/billRevenue/getPVRevenueTexList.json",
			        type: 'post',
				...
				*/
							...
							texfootStr += "<td align='right'>" + numberComma( (totBeneVal + delLastWon) ) + "</td>";
							texfootStr += "</tr>";

							elecStr += "<tr>";
							elecStr += "<th>총 발전</th>";
							elecStr += "<td align='right'>" + numberComma( ((reNetGenVal == null) ? "" : reNetGenVal) ) + "</td>";
							elecStr += "</tr>";
							elecStr += "<tr>";
		...
		//			else reRecDeal   = Math.round( Number(recDeal) );
				...
				unit_format_bill(String(totDataSet), "pvRevenueTot1", "KRW", "pv");
				unit_format_bill(String(totDataSet2), "pvRevenueTot2", "KRW", "pv");
				unit_format_bill(String(totDataSet3), "pvRevenueTot3", "KRW", "pv");*/
			...
			function getPVRevenueList(formData) {
			    $.ajax({
			        url: "/billRevenue/getPVRevenueList.json",
			        type: 'post',
			        async: false, // 동기로 처리해줌
			        data: formData,
			        success: function (result) {
						...
						if (netGenValSheetList != null && netGenValSheetList.length > 0) {
							...
							for (var i = 0; i < netGenValSheetList.length; i++) {
								...
								dt_str += "<td>" + numberComma( ((reNetGenVal == null) ? "" : reNetGenVal) ) + "</td>"; // 총 발전량
								dt_str2 += "<td>" + numberComma( ((reSmpPrice == null) ? "" : reSmpDeal) ) + "</td>"; // SMP 거래량
								dt_str3 += "<td>" + numberComma( ((reSmpPrice == null) ? "" : reSmpPrice) ) + "</td>"; // SMP 수익
								dt_str4 += "<td>" + numberComma( ((reRecDeal == null) ? "" : reRecDeal) ) + "</td>"; // REC 거래량
								dt_str5 += "<td>" + numberComma( ((reRecPrice == null) ? "" : reRecPrice) ) + "</td>"; // REC 수익
								dt_str6 += "<td>" + numberComma( ((reTotPrice == null) ? "" : reTotPrice) ) + "</td>"; //총 수익

								if (dt_col_cnt == 12) {
									dt_str += "<td>" + numberComma(dt_str_totalVal) + "</td>"; // 총 발전량
									dt_str2 += "<td>" + numberComma(dt_str2_totalVal) + "</td>"; // SMP 거래량
									dt_str3 += "<td>" + numberComma(dt_str3_totalVal) + "</td>"; // SMP 수익
									dt_str4 += "<td>" + numberComma(dt_str4_totalVal) + "</td>"; // REC 거래량
									dt_str5 += "<td>" + numberComma(dt_str5_totalVal) + "</td>"; // REC 수익
									dt_str6 += "<td>" + numberComma(dt_str6_totalVal) + "</td>"; //총 수익
							...
							// 총 합계(사용량, 발전량, 충전량, 방전량 등등)
							unit_format(String(totDataSet), "pvRevenueTot1", "KRW");
							unit_format(String(totDataSet2), "pvRevenueTot2", "KRW");
							unit_format(String(totDataSet3), "pvRevenueTot3", "KRW");
						}
			...
			function drawData_table() {
				...
				if (pvRevenue_data_pc.length > 0) {
					...
					for (var i = 0; i < dt_row; i++) {
						...
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit pk2"><span>SMP 수익 (KRW)</span></div></th>' + pvRevenue_data_pc3[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit wht"><span>REC 거래량 (kWh)</span></div></th>' + pvRevenue_data_pc4[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit pk3"><span>REC 수익 (KRW)</span></div></th>' + pvRevenue_data_pc5[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit pk1"><span>총 수익 (KRW)</span></div></th>' + pvRevenue_data_pc6[i];
						tbodyStr += '</tr>';
						tbodyStr += '</tbody>';
						tbodyStr += '</table>';
						tbodyStr += '</div>';
						...
						<div class="indiv">
								...
								<ul class="chart_total">
									<li class="ctt1">
										...
										<div class="cval" id="pvRevenueTot1"><span>0</span>KRW</div>
									</li>
									<li class="ctt2">
										<div class="ctit ctt2"><span>SMP 수익</span></div>
										<div class="cval" id="pvRevenueTot2"><span>0</span>KRW</div>
									</li>
									<li class="ctt3">
										<div class="ctit ctt3"><span>REC 수익</span></div>
										<div class="cval" id="pvRevenueTot3"><span>0</span>KRW</div>
									</li>
								</ul>
							</div>
					</div>
					...
					<div class="col-lg-12">
						<div class="indiv">
							<div class="tbl_top clear">
								...
								<ul class="fr">
									<li><a href="#;" class="save_btn" onclick="excelDownload('PV수익조회', event);">데이터저장</a>
									</li>
