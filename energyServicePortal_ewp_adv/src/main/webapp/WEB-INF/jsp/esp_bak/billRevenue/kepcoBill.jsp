...
<%@ include file="/decorators/include/taglibs.jsp"%>
		<script type="text/javascript">
			...
			var kepcoBill_head_pc = []; //  표 데이터(헤더)
			var kepcoBill_data_pc = []; //  표 데이터
			var kepcoBill_data_pc2 = []; //  표 데이터
			var kepcoBill_data_pc3 = []; //  표 데이터
			var kepcoBill_data_pc4 = []; //  표 데이터
			var kepcoBill_data_pc5 = []; //  표 데이터
			var kepcoBill_data_pc6 = []; //  표 데이터
			var kepcoBill_data_pc7 = []; //  표 데이터
			...
			function getKepcoTexBillList(formData) {
			    $.ajax({
			        url: "/billRevenue/getKepcoTexBillList.json",
			        type: 'post',
			...
			function getKepcoBillList(formData) {
			    $.ajax({
			        url: "/billRevenue/getKepcoBillList.json",
			        type: 'post',
			        async: false, // 동기로 처리해줌
			        data: formData,
			        success: function (result) {
						...
						if (sheetList != null && sheetList.length > 0) {
							for (var i = 0; i < sheetList.length; i++) {
								...
								dt_str += "<td>" + numberComma( ((reBaseRate == null) ? "" : reBaseRate) ) + "</td>"; // 기본요금(기본요금)
								dt_str2 += "<td>" + numberComma( ((rePwrFactorRate == null) ? "" : rePwrFactorRate) ) + "</td>"; // 역률(역률요금)
								dt_str3 += "<td>" + numberComma( ((reConsumeRate == null) ? "" : reConsumeRate) ) + "</td>"; // 사용요금(전력량요금)
								dt_str4 += "<td>" + numberComma( ((reTotElecRate == null) ? "" : reTotElecRate) ) + "</td>"; // 전기요금합계(전기요금계)
								dt_str5 += "<td>" + numberComma( ((reElecFund == null) ? "" : reElecFund) ) + "</td>"; // 전력기금(전력산업기반기금)
								dt_str6 += "<td>" + numberComma( ((reValAddTax == null) ? "" : reValAddTax) ) + "</td>"; // 부가세(부가가치세)
								dt_str7 += "<td>" + numberComma( ((reTotAmtBill == null) ? "" : reTotAmtBill) ) + "</td>"; // 청구금액(당월요금계)
								...
								if (dt_col_cnt == 12) {
									dt_str += "<td>" + numberComma(dt_str_totalVal) + "</td>"; // 기본요금(기본요금)
									dt_str2 += "<td>" + numberComma(dt_str2_totalVal) + "</td>"; // 역률(역률요금)
									dt_str3 += "<td>" + numberComma(dt_str3_totalVal) + "</td>"; // 사용요금(전력량요금)
									dt_str4 += "<td>" + numberComma(dt_str4_totalVal) + "</td>"; // 전기요금합계(전기요금계)
									dt_str5 += "<td>" + numberComma(dt_str5_totalVal) + "</td>"; // 전력기금(전력산업기반기금)
									dt_str6 += "<td>" + numberComma(dt_str6_totalVal) + "</td>"; // 부가세(부가가치세)
									dt_str7 += "<td>" + numberComma(dt_str7_totalVal) + "</td>"; // 청구금액(당월요금계)
						...
						if (chartList != null && chartList.length > 0) {
							...
							unit_format_bill(String(totUsage), "kepcoBillTot1", "KRW", "kepco");
							unit_format_bill(String(totUsage2), "kepcoBillTot2", "KRW", "kepco");
							unit_format_bill(String(totUsage3), "kepcoBillTot3", "KRW", "kepco");
							unit_format_bill(String(totUsage4), "kepcoBillTot4", "KRW", "kepco");
						}
			        }
			    });
			}
			...
			function drawData_table() {
				...
				if (kepcoBill_data_pc.length > 0) {
					...
					for (var i = 0; i < dt_row; i++) {
						...
						tbodyStr += '<tbody>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit ctt1"><span>기본요금 (KRW)</span></div></th>' + kepcoBill_data_pc[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit wht"><span>역률 (KRW)</span></div></th>' + kepcoBill_data_pc2[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit ctt2"><span>사용요금 (KRW)</span></div></th>' + kepcoBill_data_pc3[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit wht"><span>전기요금합계 (KRW)</span></div></th>' + kepcoBill_data_pc4[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit ctt3"><span>전력산업기반기금 (KRW)</span></div></th>' + kepcoBill_data_pc5[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit ctt4"><span>부가세 (KRW)</span></div></th>' + kepcoBill_data_pc6[i];
						tbodyStr += '</tr>';
						tbodyStr += '<tr>';
						tbodyStr += '<th><div class="ctit wht"><span>청구요금 (KRW)</span></div></th>' + kepcoBill_data_pc7[i];
						tbodyStr += '</tr>';
						tbodyStr += '</tbody>';
						tbodyStr += '</table>';
						tbodyStr += '</div>';
						...
						<div class="indiv">
							...
							<ul class="chart_total">
								<li class="ctt1">
									...
									<div class="cval" id="kepcoBillTot1"><span>0</span>KRW</div>
								</li>
								<li class="ctt2">
									<div class="ctit ctt2"><span>사용요금(역률적용)</span></div>
									<div class="cval" id="kepcoBillTot2"><span>0</span>KRW</div>
								</li>
								<li class="ctt3">
									<div class="ctit ctt3"><span>전력산업기반기금</span></div>
									<div class="cval" id="kepcoBillTot3"><span>0</span>KRW</div>
								</li>
								<li class="ctt4">
									<div class="ctit ctt4"><span>부가세</span></div>
									<div class="cval" id="kepcoBillTot4"><span>0</span>KRW</div>
								</li>
							</ul>
						</div>
					</div>
					...
					<div class="col-lg-12">
						<div class="indiv">
							<div class="tbl_top clear">
								...
								<ul class="fr">
									<li><a href="#;" class="save_btn" onclick="excelDownload('한전요금조회', event);">데이터저장</a>
									</li>
	...
	<div id="layerbox" class="dprint clear kepcoBillStatement" style="margin-top:350px;width:880px;">
		...
		<div class="lbody mt30 ">
			<table width="100%" border="0" cellpadding="0" cellspacing="0">
				...
				<tr>
					<td colspan="2" height="40" align="right" style="font-size:16px;font-weight:600;">이번 달 청구 금액은
						<span class="dp_total">2,220,000</span>원 입니다
					</td>
				</tr>
			</table>
