...
<script type="text/javascript">
	...
	function getDeviceIOEDetail(siteId, deviceId, deviceType) {
	    $.ajax({
	        url: "/device/getDeviceIOEDetail.json",
	        type: 'post',
	...
	function drawIOEDetail(result) {
		...
		ioeStr += '<th>전압(V)</th>';
		ioeStr += '<th>전력(kW)</th>';
		ioeStr += '<th>유효전력(kWh)</th>';
		ioeStr += '<th>무효전력(kWh)</th>';
		ioeStr += '</tr>';
		ioeStr += '</thead>';
		ioeStr += '<tbody>';
		ioeStr += '<tr>';
		ioeStr += '<td>' + ((ioeDetail.voltage == null) ? "-" : numberComma(toFixedNum(Number(ioeDetail.voltage) , 2))) + '</td>';
		ioeStr += '<td>' + ((ioeDetail.activePower == null) ? "-" : numberComma(toFixedNum(Number(ioeDetail.activePower) , 2))) + '</td>';
		ioeStr += '<td>' + ((ioeDetail.energy == null) ? "-" : numberComma(toFixedNum(Number(ioeDetail.energy) , 2))) + '</td>';
		ioeStr += '<td>' + ((ioeDetail.energyReactive == null) ? "-" : numberComma(toFixedNum(Number(ioeDetail.energyReactive) , 2))) + '</td>';
		ioeStr += '</tr>';
		ioeStr += '</tbody>';
		ioeStr += '</table>';
		ioeStr += '</div>';
	...
	function getDevicePCSDetail(siteId, deviceId, deviceType) {
	    $.ajax({
	        url: "/device/getDevicePCSDetail.json",
	        type: 'post',
	        async: false, // 동기로 처리해줌
	        data: {
	            siteId: siteId,
	...
	function drawPCSDetail(result) {
		...
		pcsStr += '<tbody>';
		pcsStr += '<tr>';
		pcsStr += '<td>' + ((pcsDetail.acVoltage == -1) ? "-" : numberComma(pcsDetail.acVoltage)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.acPower == -1) ? "-" : numberComma(pcsDetail.acPower)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.acFreq == -1) ? "-" : numberComma(pcsDetail.acFreq)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.acCurrent == -1) ? "-" : numberComma(pcsDetail.acCurrent)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.acPf == -1) ? "-" : numberComma(pcsDetail.acPf)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.acSetPower == -1) ? "-" : numberComma(pcsDetail.acSetPower)) + '</td>';
		pcsStr += '</tr>';
		pcsStr += '</tbody>';
		pcsStr += '</table>';
		pcsStr += '</div>';
		...
		pcsStr += '<tbody>';
		pcsStr += '<tr>';
		pcsStr += '<td>' + ((pcsDetail.dcVoltage == -1) ? "-" : numberComma(pcsDetail.dcVoltage)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.dcCurrent == -1) ? "-" : numberComma(pcsDetail.dcCurrent)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.pcsStatus == -1) ? "-" : numberComma(pcsDetail.pcsStatus)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.pcsCommand == -1) ? "-" : numberComma(pcsDetail.pcsCommand)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.todayCEnergy == -1) ? "-" : numberComma(pcsDetail.todayCEnergy)) + '</td>';
		pcsStr += '<td>' + ((pcsDetail.todayDEnergy == -1) ? "-" : numberComma(pcsDetail.todayDEnergy)) + '</td>';
		pcsStr += '</tr>';
		pcsStr += '</tbody>';
		pcsStr += '</table>';
		pcsStr += '</div>';
	...
	function getDeviceBMSDetail(siteId, deviceId, deviceType) {
	    $.ajax({
	        url: "/device/getDeviceBMSDetail.json",
	        type: 'post',
	        async: false, // 동기로 처리해줌
	        data: {
	            siteId: siteId,
	...
	function drawBMSDetail(result) {
		...
		bmsStr += '<td>' + ((bmsDetail.currSoc == -1) ? "-" : numberComma(bmsDetail.currSoc)) + '</td>';
		bmsStr += '<td>' + ((bmsDetail.sysVoltage == -1) ? "-" : numberComma(bmsDetail.sysVoltage)) + '</td>';
		bmsStr += '<td>' + ((bmsDetail.sysCurrent == -1) ? "-" : numberComma(bmsDetail.sysCurrent)) + '</td>';
	...
	function getDevicePVDetail(siteId, deviceId, deviceType) {
	    $.ajax({
	        url: "/device/getDevicePVDetail.json",
	        type: 'post',
	        async: false, // 동기로 처리해줌
	        data: {
	            siteId: siteId,
	...
	function drawPVDetail(result) {
		...
		pvStr += '<td>' + '-' + '</td>';
		pvStr += '<td>' + ((pvDetail.totalPower == -1) ? "-" : numberComma(pvDetail.totalPower) + "Wh") + '</td>';
		pvStr += '<td>' + ((pvDetail.todayPower == -1) ? "-" : numberComma(pvDetail.todayPower) + "Wh") + '</td>';
		pvStr += '</tr>';
		pvStr += '</tbody>';
		pvStr += '</table>';
		pvStr += '</div>';
