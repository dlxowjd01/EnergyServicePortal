...
const firstAjax = () => {
	...
	monthlyChartDraw(siteSids, false);
	dailyChartDraw(siteSids, false);
...
const minAjax = async () => {
	...
	new Promise(resolve => {
		...
		let acPowerSum = 0, capacitySum = 0, invertorCount = 0;
		response.forEach((resData, index) => {
			if (!isEmpty(resData)) {
				if (index === 0) {
					//월간 차트
					let energy = 0, money = 0;
 ...
 */
const monthlyChartDraw = async (siteSids, renewal) => {
	const targetApi = new Array();
	...
	if (renewal) {
		...
		targetApi.push($.ajax({
			...
			data: JSON.stringify({
				sid: siteSids.toString(),
				startTime: Number(yearData.startTime),
				endTime: Number(yearData.endTime),
				interval: 'month',
	...
	new Promise(resolve => {
		...
		const sumObj = { chargeSum: 0, dischargeSum: 0, pvSum: 0 };
		response.forEach((resData, index) => {
		...
		new Promise(resolve => {
			...
			Object.entries(sumObj).forEach(([key, value]) => {
				...
				if (key === 'pvSum') {
					str += '<li class="pv">' + i18nManager.tr('dashboard_photovoltaic') + ' : ' + displayNumberFixedDecimal(Math.floor(value / 1000), 'kWh', 3, 2).join(' ') + '</li>';
				}
			});
 ...
 * @returns {Promise<void | boolean>}
 */
const dailyChartDraw = async (siteSids, renewal) => {
	const today = new Date();
