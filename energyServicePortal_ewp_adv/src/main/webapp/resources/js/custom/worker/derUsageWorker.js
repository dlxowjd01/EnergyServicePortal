...
function init(config){
    ...
    usage_head_pc.length = 0; // 표 영역 헤더
    real_data_pc.length = 0; // 한전 사용량 표 데이터
    ess_data_pc.length = 0; //  ess 사용량 표 데이터
    pv_data_pc.length = 0; //  pv 사용량 표 데이터
    total_data_pc.length = 0; //  사용량 합계 표 데이터
}
...
function derUsageListUI(result) {
    ...
    if (!(kepcoUsageSheetList == null && essUsageListSheetList == null && pvUsageListSheetList == null)) {
        ...
        for (var i = 0; i < loopCntSheetList.length; i++) {
            ...
            dt_str += "<td>" + numberComma( (( isEmpty(reKepcoUsage)) ? "" : reKepcoUsage) ) + "</td>"; // 한전 사용량
            dt_str2 += "<td>" + numberComma( (( isEmpty(reEssUsage)) ? "" : reEssUsage) ) + "</td>"; // ess 사용량
            dt_str3 += "<td>" + numberComma( (( isEmpty(rePvUsage)) ? "" : rePvUsage) ) + "</td>"; // pv 사용량
            dt_str4 += "<td>" + numberComma( (( isEmpty(reTotalUsage)) ? "" : reTotalUsage) ) + "</td>"; // 사용량 합계
