...
import kr.co.esp.common.util.EnertalkApiUtil.Period;
import kr.co.esp.common.util.EnertalkApiUtil.TimeType;
import kr.co.esp.common.util.EnertalkApiUtil.UsageType;
...
public class ApiController {
	...
	@RequestMapping(value = {"/v1/alarm"}, method = RequestMethod.POST, produces = MediaType.TEXT_HTML_VALUE)
	/*** 12.13 이우람 수정 ***/
	public void getPrototype(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap, //
							 @RequestParam String siteId, // 사이트ID
							 @RequestParam String deviceId, // 장치ID
							 @RequestParam Integer deviceType, // 장치구분(1:PCS,2:BMS,3:PV,4:부하측정기기,5:PV모니터링기기,6:ESS모니터링기기,7:iSmart,8:총량기기,9:AMI)
							 @RequestParam Long alarmTime, // 알람발생일시(형식:YYYYMMDDhhmmss) -> 기준일시에 저장
							 @RequestParam Integer alarmType, // 알람구분(1:비상,2:주의)
		...
		try {
			...
			String msg = CommonUtils.StringReplace(alarmMsg);
			parameter.put("alarmMsg", msg); // alarm message
			...
			// SMS 전송 (한재종)
			if (device != null) {
				String alarmTimeStr = CommonUtils.convertDateFormat(stdDate, "yyyyMMddHHmmss");
				smsService.sendAlarmMessage(siteId, deviceId, device.get("device_name"), alarmTimeStr, Integer.toString(alarmType), msg);
			}
	...
	@RequestMapping("/getSoc.json")
	public @ResponseBody Map<String, Object> getSoc(@RequestParam Map<String, Object> param, HttpServletRequest request) throws Exception {
		logger.debug("/getSoc.json");
...
//							}
	...
	@RequestMapping("/getPeak.json")
	public @ResponseBody Map<String, Object> getPeak(@RequestParam Map<String, Object> param) throws Exception {
		logger.debug("/getPeak.json");
		logger.debug("param ::::: " + param.toString());
	...
	public @ResponseBody
	Integer loginUser(HttpSession session, String userId, String userPw) throws Exception {
		...
		Map<String, Object> result = cmmloginService.getUserDetailPlain(param);
	...
	public @ResponseBody
	Map<String, Object> getUserDetail(String userId, String userPw) throws Exception {
